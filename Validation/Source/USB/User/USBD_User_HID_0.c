...
#include "MW_CV_Config.h"
        ...
        ((MW_CV_USBH != 0U) && (MW_CV_USBH_HID != 0U)))
        ...
        case 0:
          switch (req) {
            ...
            case USBD_HID_REQ_EP_INT:         // Called after USBD_HID_GetReportTrigger to signal
                                              // data obtained.
              mw_cv_usbd_hid0.cb_get_report = true;
              break;
...
bool USBD_HID0_SetReport (uint8_t rtype, uint8_t req, uint8_t rid, const uint8_t *buf, int32_t len) {
  ...
  switch (rtype) {
    case HID_REPORT_OUTPUT:
      if (*buf == 'S') {
        mw_cv_usbd_hid0.cb_set_report = true;
      } else {
        set_buf[0] = *buf;
        USBD_HID_GetReportTrigger (0U, 0U, set_buf, len);
      }
      break;

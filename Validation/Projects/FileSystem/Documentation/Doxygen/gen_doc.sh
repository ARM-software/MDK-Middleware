...
projectName=$(grep -E "PROJECT_NAME\s+=" FS_Validation.dxy.in | sed -r -e 's/[^"]*"([^"]+)".*/\1/')
projectNumberFull="${VERSION_FULL}"
projectNumber="${projectNumberFull%+*}"
datetime=$(date -u +'%a %b %e %Y %H:%M:%S')
year=$(date -u +'%Y')

sed -e "s/{projectNumber}/${projectNumber}/" FS_Validation.dxy.in > FS_Validation.dxy

echo "\"${UTILITY_DOXYGEN}\" FS_Validation.dxy"
"${UTILITY_DOXYGEN}" FS_Validation.dxy

 ...
 *------------------------------------------------------------------------------
 * Name:    USBD_User_CDC_ACM_UART_%Instance%.c
 ...
 *
 * USBD_User_CDC_ACM_UART_%Instance%.c implements the application specific
 ...
 *     the UART is sent over USB using the CDC%Instance%_ACM_UART_to_USB_Thread thread.
 *   USB -> UART:
 *     While the UART transmit is not busy, data transmission on the UART is
 *     started in the USBD_CDC%Instance%_ACM_DataReceived callback as soon as data is
 ...
 *     USBD_CDC%Instance%_ACM_DataReceived callback as soon as new data is received
 *     on the USB.
 *
 * The following constants in this module affect the module functionality:
 *
 *  - UART_PORT:        specifies UART Port
 *      default value:  %Instance% (=UART%Instance%)
...
#define  UART_PORT              %Instance%       // UART Port number
...
extern   void                   CDC%Instance%_ACM_UART_to_USB_Thread (void const *arg);
#endif
...
static void UART_Callback (uint32_t event) {
  ...
  if (event & ARM_USART_EVENT_SEND_COMPLETE) {
    // USB -> UART
    cnt = USBD_CDC_ACM_ReadData(%Instance%U, uart_tx_buf, UART_BUFFER_SIZE);
    if (cnt > 0) {
...
#else
__NO_RETURN        void CDC%Instance%_ACM_UART_to_USB_Thread (void const *arg) {
#endif
  ...
  for (;;) {
    ...
    if (ptrUART->GetStatus().rx_busy != 0U) {
      ...
      if (cnt > 0) {
        ...
        cnt = USBD_CDC_ACM_WriteData(%Instance%U, (uart_rx_buf + ((uint32_t)usb_tx_cnt & (UART_BUFFER_SIZE - 1))), cnt);
        if (cnt > 0) {
          usb_tx_cnt += cnt;
        }
      }
...
#else
extern const osThreadDef_t os_thread_def_CDC%Instance%_ACM_UART_to_USB_Thread;
osThreadDef (CDC%Instance%_ACM_UART_to_USB_Thread, osPriorityNormal, 1U, 0U);
#endif
...
#else
  cdc_acm_bridge_tid = osThreadCreate (osThread (CDC%Instance%_ACM_UART_to_USB_Thread), NULL);
#endif
}

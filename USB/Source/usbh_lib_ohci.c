 ...
 * $Revision:    V2.21
 *
 * Driver:       Driver_USBH_OHCI
 * Configured:   via RTE_Device.h configuration file
 * Project:      USB Host generic OHCI Driver
 * -----------------------------------------------------------------------------
 ...
 *  Version 2.19
 *    Fixed HardFault happening during debugging
 *  Version 2.18
 *    Removed redundant transfer num information 
 *  Version 2.17
 *    Corrected USBH_OHCI_TD_Enqueue function (if pipe received stall it would 
 *    prevent further communication while halt status of Endpoint Descriptor 
 *    was active)
 *  Version 2.16
 *    Corrected USBH_OHCI_ED_GetLast function
 *    Corrected interrupt pipes stop processing when last added interrupt pipe 
 *    is deleted
 *  Version 2.15
 *    Corrected number of bytes transferred
 *  Version 2.14
 *    Corrected transferred bytes calculation in IRQ case that 
 *    CurrentBufferPointer of Transfer Descriptor was not 0 after transfer 
 *    (this was observed with MW v7.4.1 and some USB Flash Disks which after 
 *    returning 18 bytes device descriptor the CurrentBufferPointer was not 0, 
 *    but ConditionCode is 0, also exact number of transferred bytes which 
 *    was 18 is not possible to get from Transfer Descriptor)
 *  Version 2.13
 *    MISRA compliance
 *  Version 2.12
 ...
 *  Version 2.8
 *    Fixed interrupt pipe polling interval
 *  Version 2.0
 *    Initial release for USB Host CMSIS Driver API v2.0
 */

#include "usbh_lib_ohci.h"
...
#define OHCI_MAX_INSTANCE_NUM 1         // Number of supported instances


/* USBH Driver ****************************************************************/

#define OHCI_ARM_USBH_DRIVER_VERSION ARM_DRIVER_VERSION_MAJOR_MINOR(2,21)

/* Driver Version */
static const ARM_DRIVER_VERSION usbh_driver_version = { ARM_USBH_API_VERSION, OHCI_ARM_USBH_DRIVER_VERSION };

/* Driver Capabilities */
static ARM_USBH_CAPABILITIES usbh_driver_capabilities[OHCI_MAX_INSTANCE_NUM] = {
  {
...
static USBH_OHCI_Registers_t           *usbh_ohci_reg_ptr[OHCI_MAX_INSTANCE_NUM];
static ARM_USBH_SignalPortEvent_t       signal_port_event[OHCI_MAX_INSTANCE_NUM];
static ARM_USBH_SignalPipeEvent_t       signal_pipe_event[OHCI_MAX_INSTANCE_NUM];
...
static   void (* const USBH_IRQ_Handler[OHCI_MAX_INSTANCE_NUM]) (void)= {
    USBH0_IRQ_Handler
#if (OHCI_MAX_INSTANCE_NUM >= 2)
  , USBH1_IRQ_Handler
#endif
...
static bool USBH_OHCI_TD_Dequeue (uint8_t ctrl, USBH_OHCI_ED *ptr_ED, USBH_OHCI_TD *ptr_TD) {
  ...
  (void)USBH_Delay (1U);                // Wait ~1 ms to be sure that endpoint descriptor is not being processed
...
static void USBH_OHCI_TI_ClearAll (uint8_t ctrl) {
  ...
  memset((*usbh_ohci_ptr[ctrl])->ptr_TI, 0, (*usbh_pipe_num_ptr[ctrl])*sizeof(USBH_TRANSFER_INFO_t));
}

/**
  \fn          USBH_TRANSFER_INFO_t *USBH_OHCI_TI_GetFree (uint8_t ctrl)
...
*/
static USBH_TRANSFER_INFO_t *USBH_OHCI_TI_GetFree (uint8_t ctrl) {
...
/**
  \fn          USBH_TRANSFER_INFO_t *USBH_OHCI_TI_Find_TD (uint8_t ctrl, USBH_OHCI_TD *ptr_TD)
...
*/
static USBH_TRANSFER_INFO_t *USBH_OHCI_TI_Find_TD (uint8_t ctrl, USBH_OHCI_TD *ptr_TD) {
  USBH_TRANSFER_INFO_t *ptr_TI;
  uint32_t              cnt;
...
/**
  \fn          USBH_TRANSFER_INFO_t *USBH_OHCI_TI_Find_ED (uint8_t ctrl, USBH_OHCI_ED ptr_ED)
  ...
  \return                  Pointer to Transfer Info entry, no corresponding entry or function failed
               ...
               value = 0:  no Transfer Info entry found or function failed
*/
static USBH_TRANSFER_INFO_t *USBH_OHCI_TI_Find_ED (uint8_t ctrl, USBH_OHCI_ED *ptr_ED) {
  USBH_TRANSFER_INFO_t *ptr_TI;
  uint32_t              cnt;
...
extern
ARM_DRIVER_USBH Driver_USBH0_OHCI_LIB;
ARM_DRIVER_USBH Driver_USBH0_OHCI_LIB = {
  USBH0_HW_GetVersion,
...
#if (OHCI_MAX_INSTANCE_NUM >= 2)
extern
ARM_DRIVER_USBH Driver_USBH1_OHCI_LIB;
ARM_DRIVER_USBH Driver_USBH1_OHCI_LIB = {
  USBH1_HW_GetVersion,
...
#if (OHCI_MAX_INSTANCE_NUM >= 3)
extern
ARM_DRIVER_USBH Driver_USBH2_OHCI_LIB;
ARM_DRIVER_USBH Driver_USBH2_OHCI_LIB = {
  USBH2_HW_GetVersion,
...
#if (OHCI_MAX_INSTANCE_NUM >= 4)
extern
ARM_DRIVER_USBH Driver_USBH3_OHCI_LIB;
ARM_DRIVER_USBH Driver_USBH3_OHCI_LIB = {
  USBH3_HW_GetVersion,

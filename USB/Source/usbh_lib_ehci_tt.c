/* -----------------------------------------------------------------------------
 * Copyright (c) 2013-2023 Arm Limited (or its affiliates). All rights reserved.
 *
 *
 * $Date:        17. August 2022
 * $Revision:    V2.23
 *
 * Driver:       Driver_USBH_EHCI
 * Configured:   via RTE_Device.h configuration file
 * Project:      USB Host EHCI Controller Driver for customized EHCI with
 *               internal Transaction Translator (TT)
 ...
 *   ---------------------                -----
 *   Connect to hardware via Driver_USBH# = x
 *   USB Host controller interface        = EHCI
 * -------------------------------------------------------------------------- */

/* History:
 *  Version 2.23
 *    Corrected code to compile with Arm Compiler 6 without warnings
 *  Version 2.22
 *    Changed configuration include file name
 *  Version 2.21
 *    Added full-speed isochronous support
 *    Changed transfer descriptor allocation to pipe creation
 *  Version 2.20
 *    Corrected transfer for sizes larger than 16kB
 *  Version 2.19
 *    Removed redundant transfer num information 
 *  Version 2.18
 *    Corrected asynchronous simultaneous transfers functionality
 *  Version 2.17
 *    Corrected USB Controller power off procedure
 *  Version 2.16
 *    MISRA compliance
 *  Version 2.15
 *    Forced data toggle to DATA0 for sending SETUP packet
 *  Version 2.14
 *    Renamed USBH_osDelay with USBH_Delay
 *  Version 2.13
 *    Updated for ARM Compiler 6 compliance
 *  Version 2.12
 *    Removed suspend and resume events as HCD only drives suspend or resume
 *    on the bus
 *    Corrected over-current signaling
 *    Corrected remote wakeup signaling
 *  Version 2.10
 *    Fixed interrupt pipe creation and deletion
 *  Version 2.0
 *    Initial release for USB Host CMSIS Driver API v2.0
 */

#include "usbh_lib_ehci_tt.h"
...
/* USBH Driver Settings *******************************************************/

#define EHCI_MAX_INSTANCE_NUM 2         // Number of supported instances


/* USBH Driver ****************************************************************/

#define EHCI_ARM_USBH_DRIVER_VERSION ARM_DRIVER_VERSION_MAJOR_MINOR(2,23)

/* Driver Version */
static const ARM_DRIVER_VERSION usbh_driver_version = { ARM_USBH_API_VERSION, EHCI_ARM_USBH_DRIVER_VERSION };

/* Driver Capabilities */
static ARM_USBH_CAPABILITIES usbh_driver_capabilities[EHCI_MAX_INSTANCE_NUM] = {
  {
...
#if (defined(ARM_USBH_API_VERSION) && (ARM_USBH_API_VERSION >= 0x202))
  , 0U          /* reserved bits                   */
#endif
  }
#if (EHCI_MAX_INSTANCE_NUM >= 2)
, {
    0x3FFFU,    /* Root HUB available Ports Mask   */
    0U,         /* Automatic SPLIT packet handling */
    1U,         /* Signal Connect event            */
...
static USBH_EHCI_Registers_t           *usbh_ehci_reg_ptr[EHCI_MAX_INSTANCE_NUM];
static ARM_USBH_SignalPortEvent_t       signal_port_event[EHCI_MAX_INSTANCE_NUM];
static ARM_USBH_SignalPipeEvent_t       signal_pipe_event[EHCI_MAX_INSTANCE_NUM];

static uint32_t pfl_size[EHCI_MAX_INSTANCE_NUM];

static uint32_t port_act[EHCI_MAX_INSTANCE_NUM];
...
#endif
#if (EHCI_MAX_INSTANCE_NUM >= 3)
static   void          USBH2_IRQ_Handler (void);
#endif
#if (EHCI_MAX_INSTANCE_NUM >= 4)
static   void          USBH3_IRQ_Handler (void);
#endif

static   void (* const USBH_IRQ_Handler[EHCI_MAX_INSTANCE_NUM]) (void)= {
    USBH0_IRQ_Handler
#if (EHCI_MAX_INSTANCE_NUM >= 2)
  , USBH1_IRQ_Handler
#endif
...
/**
  ...
  \return      true = success, false = fail
*/
static bool USBH_EHCI_qTD_Enqueue (uint8_t ctrl, USBH_TRANSFER_INFO_t *ptr_TI, USBH_EHCI_qH *ptr_qH, USBH_EHCI_qTD *ptr_qTD, uint32_t packet, uint8_t *data, uint32_t num) {
...
/**
  ...
  \return      true = success, false = fail
*/
static bool USBH_EHCI_IsoTransferActivate (uint8_t ctrl, USBH_TRANSFER_INFO_t *ptr_TI, void *ptr_pe, uint8_t *data, uint32_t num) {
...
static void USBH_EHCI_TI_ClearAll (uint8_t ctrl) {
  ...
  memset((*usbh_ehci_ptr[ctrl])->ptr_TI, 0, (*usbh_pipe_num_ptr[ctrl])*sizeof(USBH_TRANSFER_INFO_t));
}

/**
  \fn          USBH_TRANSFER_INFO_t *USBH_EHCI_TI_GetFree (uint8_t ctrl)
...
*/
static USBH_TRANSFER_INFO_t *USBH_EHCI_TI_GetFree (uint8_t ctrl) {
  USBH_TRANSFER_INFO_t *ptr_TI;
  uint32_t              cnt;
...
/**
  \fn          USBH_TRANSFER_INFO_t *USBH_EHCI_TI_Find_qTD (uint8_t ctrl, USBH_EHCI_qTD *ptr_qTD)
...
*/
static USBH_TRANSFER_INFO_t *USBH_EHCI_TI_Find_qTD (uint8_t ctrl, USBH_EHCI_qTD *ptr_qTD) {
  USBH_TRANSFER_INFO_t *ptr_TI;
  uint32_t              cnt;
...
/**
  \fn          USBH_TRANSFER_INFO_t *USBH_EHCI_TI_Find_qH (uint8_t ctrl, USBH_EHCI_qH *ptr_qH)
  ...
  \return                  Pointer to Transfer Info entry, no corresponding entry or function failed
               ...
               value = 0:  no Transfer Info entry found or function failed
*/
static USBH_TRANSFER_INFO_t *USBH_EHCI_TI_Find_qH (uint8_t ctrl, USBH_EHCI_qH *ptr_qH) {
  USBH_TRANSFER_INFO_t *ptr_TI;
  uint32_t              cnt;
...
/**
  \fn          USBH_TRANSFER_INFO_t *USBH_EHCI_TI_Find_PE (uint8_t ctrl, void *ptr_pe)
  ...
  \return                  Pointer to Transfer Info entry, no corresponding entry or function failed
               ...
               value = 0:  no Transfer Info entry found or function failed
*/
static USBH_TRANSFER_INFO_t *USBH_EHCI_TI_Find_PE (uint8_t ctrl, void *ptr_pe) {
  USBH_TRANSFER_INFO_t *ptr_TI;
  uint32_t              cnt;
...
#endif
#if   (EHCI_MAX_INSTANCE_NUM >= 3)
static ARM_DRIVER_VERSION USBH2_HW_GetVersion (void) { return usbh_driver_version; }
#endif
#if   (EHCI_MAX_INSTANCE_NUM >= 4)
static ARM_DRIVER_VERSION USBH3_HW_GetVersion (void) { return usbh_driver_version; }
#endif
...
#endif
#if   (EHCI_MAX_INSTANCE_NUM >= 3)
static ARM_USBH_CAPABILITIES USBH2_HW_GetCapabilities (void)         { return USBH_HW_GetCapabilities (2); }
#endif
#if   (EHCI_MAX_INSTANCE_NUM >= 4)
static ARM_USBH_CAPABILITIES USBH3_HW_GetCapabilities (void)         { return USBH_HW_GetCapabilities (3); }
#endif
...
#endif
#if   (EHCI_MAX_INSTANCE_NUM >= 3)
static int32_t USBH2_HW_Initialize (ARM_USBH_SignalPortEvent_t cb_port_event, ARM_USBH_SignalPipeEvent_t cb_pipe_event) { return USBH_HW_Initialize (2, cb_port_event, cb_pipe_event); }
#endif
#if   (EHCI_MAX_INSTANCE_NUM >= 4)
static int32_t USBH3_HW_Initialize (ARM_USBH_SignalPortEvent_t cb_port_event, ARM_USBH_SignalPipeEvent_t cb_pipe_event) { return USBH_HW_Initialize (3, cb_port_event, cb_pipe_event); }
#endif
...
#endif
#if   (EHCI_MAX_INSTANCE_NUM >= 3)
static int32_t USBH2_HW_Uninitialize (void) { return USBH_HW_Uninitialize(2); }
#endif
#if   (EHCI_MAX_INSTANCE_NUM >= 4)
static int32_t USBH3_HW_Uninitialize (void) { return USBH_HW_Uninitialize(3); }
#endif
...
#endif
#if   (EHCI_MAX_INSTANCE_NUM >= 3)
static int32_t USBH2_HW_PowerControl (ARM_POWER_STATE state) { return USBH_HW_PowerControl (2, state); }
#endif
#if   (EHCI_MAX_INSTANCE_NUM >= 4)
static int32_t USBH3_HW_PowerControl (ARM_POWER_STATE state) { return USBH_HW_PowerControl (3, state); }
#endif
...
#endif
#if   (EHCI_MAX_INSTANCE_NUM >= 3)
static int32_t USBH2_HW_PortVbusOnOff (uint8_t port, bool vbus) { return USBH_HW_PortVbusOnOff(2, port, vbus); }
#endif
#if   (EHCI_MAX_INSTANCE_NUM >= 4)
static int32_t USBH3_HW_PortVbusOnOff (uint8_t port, bool vbus) { return USBH_HW_PortVbusOnOff(3, port, vbus); }
#endif
...
#endif
#if   (EHCI_MAX_INSTANCE_NUM >= 3)
static int32_t USBH2_HW_PortReset (uint8_t port) { return USBH_HW_PortReset (2, port); }
#endif
#if   (EHCI_MAX_INSTANCE_NUM >= 4)
static int32_t USBH3_HW_PortReset (uint8_t port) { return USBH_HW_PortReset (3, port); }
#endif
...
#endif
#if   (EHCI_MAX_INSTANCE_NUM >= 3)
static int32_t USBH2_HW_PortSuspend (uint8_t port) { return USBH_HW_PortSuspend (2, port); }
#endif
#if   (EHCI_MAX_INSTANCE_NUM >= 4)
static int32_t USBH3_HW_PortSuspend (uint8_t port) { return USBH_HW_PortSuspend (3, port); }
#endif
...
#endif
#if   (EHCI_MAX_INSTANCE_NUM >= 3)
static int32_t USBH2_HW_PortResume (uint8_t port) { return USBH_HW_PortResume (2, port); }
#endif
#if   (EHCI_MAX_INSTANCE_NUM >= 4)
static int32_t USBH3_HW_PortResume (uint8_t port) { return USBH_HW_PortResume (3, port); }
#endif
...
#endif
#if   (EHCI_MAX_INSTANCE_NUM >= 3)
static ARM_USBH_PORT_STATE USBH2_HW_PortGetState (uint8_t port) { return USBH_HW_PortGetState (2, port); }
#endif
#if   (EHCI_MAX_INSTANCE_NUM >= 4)
static ARM_USBH_PORT_STATE USBH3_HW_PortGetState (uint8_t port) { return USBH_HW_PortGetState (3, port); }
#endif
...
static ARM_USBH_PIPE_HANDLE USBH_HW_PipeCreate (uint8_t ctrl, uint8_t dev_addr, uint8_t dev_speed, uint8_t hub_addr, uint8_t hub_port, uint8_t ep_addr, uint8_t ep_type, uint16_t ep_max_packet_size, uint8_t ep_interval) {
  ...
  if (ep_type != ARM_USB_ENDPOINT_ISOCHRONOUS){ // If Control, Bulk or Interrupt pipe
    ...
    memset(ptr_TI, 0, sizeof(USBH_TRANSFER_INFO_t));
    ...
    if (dev_speed == USB_SPEED_HIGH) {          // High-speed isochronous
...
#endif
#if   (EHCI_MAX_INSTANCE_NUM >= 3)
static ARM_USBH_PIPE_HANDLE USBH2_HW_PipeCreate (uint8_t dev_addr, uint8_t dev_speed, uint8_t hub_addr, uint8_t hub_port, uint8_t ep_addr, uint8_t ep_type, uint16_t ep_max_packet_size, uint8_t  ep_interval) { return USBH_HW_PipeCreate (2, dev_addr, dev_speed, hub_addr, hub_port, ep_addr, ep_type, ep_max_packet_size, ep_interval); }
#endif
#if   (EHCI_MAX_INSTANCE_NUM >= 4)
static ARM_USBH_PIPE_HANDLE USBH3_HW_PipeCreate (uint8_t dev_addr, uint8_t dev_speed, uint8_t hub_addr, uint8_t hub_port, uint8_t ep_addr, uint8_t ep_type, uint16_t ep_max_packet_size, uint8_t  ep_interval) { return USBH_HW_PipeCreate (3, dev_addr, dev_speed, hub_addr, hub_port, ep_addr, ep_type, ep_max_packet_size, ep_interval); }
#endif
...
#endif
#if   (EHCI_MAX_INSTANCE_NUM >= 3)
static int32_t USBH2_HW_PipeModify (ARM_USBH_PIPE_HANDLE pipe_hndl, uint8_t dev_addr, uint8_t dev_speed, uint8_t hub_addr, uint8_t hub_port, uint16_t ep_max_packet_size) { return USBH_HW_PipeModify (2, pipe_hndl, dev_addr, dev_speed, hub_addr, hub_port, ep_max_packet_size); }
#endif
#if   (EHCI_MAX_INSTANCE_NUM >= 4)
static int32_t USBH3_HW_PipeModify (ARM_USBH_PIPE_HANDLE pipe_hndl, uint8_t dev_addr, uint8_t dev_speed, uint8_t hub_addr, uint8_t hub_port, uint16_t ep_max_packet_size) { return USBH_HW_PipeModify (3, pipe_hndl, dev_addr, dev_speed, hub_addr, hub_port, ep_max_packet_size); }
#endif
...
#endif
#if   (EHCI_MAX_INSTANCE_NUM >= 3)
static int32_t USBH2_HW_PipeTransferAbort (ARM_USBH_PIPE_HANDLE pipe_hndl) { return USBH_HW_PipeTransferAbort (2, pipe_hndl); }
#endif
#if   (EHCI_MAX_INSTANCE_NUM >= 4)
static int32_t USBH3_HW_PipeTransferAbort (ARM_USBH_PIPE_HANDLE pipe_hndl) { return USBH_HW_PipeTransferAbort (3, pipe_hndl); }
#endif
...
#endif
#if   (EHCI_MAX_INSTANCE_NUM >= 3)
static int32_t USBH2_HW_PipeDelete (ARM_USBH_PIPE_HANDLE pipe_hndl) { return USBH_HW_PipeDelete (2, pipe_hndl); }
#endif
#if   (EHCI_MAX_INSTANCE_NUM >= 4)
static int32_t USBH3_HW_PipeDelete (ARM_USBH_PIPE_HANDLE pipe_hndl) { return USBH_HW_PipeDelete (3, pipe_hndl); }
#endif
...
#endif
#if   (EHCI_MAX_INSTANCE_NUM >= 3)
static int32_t USBH2_HW_PipeReset (ARM_USBH_PIPE_HANDLE pipe_hndl) { return USBH_HW_PipeReset (2, pipe_hndl); }
#endif
#if   (EHCI_MAX_INSTANCE_NUM >= 4)
static int32_t USBH3_HW_PipeReset (ARM_USBH_PIPE_HANDLE pipe_hndl) { return USBH_HW_PipeReset (3, pipe_hndl); }
#endif
...
#endif
#if   (EHCI_MAX_INSTANCE_NUM >= 3)
static int32_t USBH2_HW_PipeTransfer (ARM_USBH_PIPE_HANDLE pipe_hndl, uint32_t packet, uint8_t *data, uint32_t num) { return USBH_HW_PipeTransfer (2, pipe_hndl, packet, data, num); }
#endif
#if   (EHCI_MAX_INSTANCE_NUM >= 4)
static int32_t USBH3_HW_PipeTransfer (ARM_USBH_PIPE_HANDLE pipe_hndl, uint32_t packet, uint8_t *data, uint32_t num) { return USBH_HW_PipeTransfer (3, pipe_hndl, packet, data, num); }
#endif
...
static uint32_t USBH_HW_PipeTransferGetResult (uint8_t ctrl, ARM_USBH_PIPE_HANDLE pipe_hndl) {
  const USBH_TRANSFER_INFO_t *ptr_TI;
...
#endif
#if   (EHCI_MAX_INSTANCE_NUM >= 3)
static uint32_t USBH2_HW_PipeTransferGetResult (ARM_USBH_PIPE_HANDLE pipe_hndl) { return USBH_HW_PipeTransferGetResult (2, pipe_hndl); }
#endif
#if   (EHCI_MAX_INSTANCE_NUM >= 4)
static uint32_t USBH3_HW_PipeTransferGetResult (ARM_USBH_PIPE_HANDLE pipe_hndl) { return USBH_HW_PipeTransferGetResult (3, pipe_hndl); }
#endif
...
#endif
#if   (EHCI_MAX_INSTANCE_NUM >= 3)
static uint16_t USBH2_HW_GetFrameNumber (void) { return USBH_HW_GetFrameNumber (2); }
#endif
#if   (EHCI_MAX_INSTANCE_NUM >= 4)
static uint16_t USBH3_HW_GetFrameNumber (void) { return USBH_HW_GetFrameNumber (3); }
#endif
...
#endif
#if   (EHCI_MAX_INSTANCE_NUM >= 3)
static void USBH2_IRQ_Handler (void) { USBH_HW_IRQ_Handler (2); }
#endif
#if   (EHCI_MAX_INSTANCE_NUM >= 4)
static void USBH3_IRQ_Handler (void) { USBH_HW_IRQ_Handler (3); }
#endif

extern 
ARM_DRIVER_USBH Driver_USBH0_EHCI_LIB;
ARM_DRIVER_USBH Driver_USBH0_EHCI_LIB = {
  USBH0_HW_GetVersion,
...
#if (EHCI_MAX_INSTANCE_NUM >= 2)
extern 
ARM_DRIVER_USBH Driver_USBH1_EHCI_LIB;
ARM_DRIVER_USBH Driver_USBH1_EHCI_LIB = {
  USBH1_HW_GetVersion,
...
#if (EHCI_MAX_INSTANCE_NUM >= 3)
extern 
ARM_DRIVER_USBH Driver_USBH2_EHCI_LIB;
ARM_DRIVER_USBH Driver_USBH2_EHCI_LIB = {
  USBH2_HW_GetVersion,
  USBH2_HW_GetCapabilities,
  USBH2_HW_Initialize,
  USBH2_HW_Uninitialize,
  USBH2_HW_PowerControl,
  USBH2_HW_PortVbusOnOff,
  USBH2_HW_PortReset,
  USBH2_HW_PortSuspend,
  USBH2_HW_PortResume,
  USBH2_HW_PortGetState,
  USBH2_HW_PipeCreate,
  USBH2_HW_PipeModify,
  USBH2_HW_PipeDelete,
  USBH2_HW_PipeReset,
  USBH2_HW_PipeTransfer,
  USBH2_HW_PipeTransferGetResult,
  USBH2_HW_PipeTransferAbort,
  USBH2_HW_GetFrameNumber
};
#endif

#if (EHCI_MAX_INSTANCE_NUM >= 4)
extern 
ARM_DRIVER_USBH Driver_USBH3_EHCI_LIB;
ARM_DRIVER_USBH Driver_USBH3_EHCI_LIB = {
  USBH3_HW_GetVersion,
  USBH3_HW_GetCapabilities,
  USBH3_HW_Initialize,
  USBH3_HW_Uninitialize,
  USBH3_HW_PowerControl,
  USBH3_HW_PortVbusOnOff,
  USBH3_HW_PortReset,
  USBH3_HW_PortSuspend,
  USBH3_HW_PortResume,
  USBH3_HW_PortGetState,
  USBH3_HW_PipeCreate,
  USBH3_HW_PipeModify,
  USBH3_HW_PipeDelete,
  USBH3_HW_PipeReset,
  USBH3_HW_PipeTransfer,
  USBH3_HW_PipeTransferGetResult,
  USBH3_HW_PipeTransferAbort,
  USBH3_HW_GetFrameNumber
};
#endif
